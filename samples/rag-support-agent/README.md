...
- Go and Python source checks flag intentionally risky sample code
...
```bash
go run ./cmd/infraseal scan --profile full --target samples/rag-support-agent --include-tool-details
...
go run ./cmd/infraseal --config samples/rag-support-agent/.infraseal/infraseal.yaml scan --check code-security --target app/
```

The first command scans the whole sample directory. InfraSeal discovers the sample config, then evaluates prompts, exported chatbot responses, approved evidence, Go and Python workload code, the agent skill manifest, and Terraform plan JSON.
